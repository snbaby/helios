...
public class DetectController {
	...
	@ResponseBody
	public ResponseEntity<ResponseSuccessResult> edit(HttpServletRequest request, @RequestBody JSONObject jsb) {
		log.debug("jsb:{}", jsb);
		String name = jsb.getString("name");
		String id = jsb.getString("id");
		String ip = jsb.getString("ip");

		detectService.editDetect(name, id, ip, request.getSession().getAttribute("userId").toString());

		logService.addLog(request, "修改加侦测器：" + jsb.toJSONString());
		ResponseSuccessResult responseResult = new ResponseSuccessResult(HttpStatus.OK.value(), "success");
		return new ResponseEntity<>(responseResult, HttpStatus.OK);
	}
